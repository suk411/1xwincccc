...
const Earn = () => {
  ...
  const [subDateFilter, setSubDateFilter] = useState<"today" | "yesterday" | "month">("today");
  ...
  useEffect(() => {
    ...
    setLevelVal("All");
    setSearchUid("");
    fetchAgencyNewSub(startStr, endStr);
          ...
          <div className="x-page">
            ...
            <div className="bet-container-sticky"><div className="van-sticky"><div>
              <div ref={scrollRef} className="flex gap-2 overflow-x-auto px-3 py-2 hide-scrollbar scroll-snap-x" style={{ scrollBehavior: "smooth", WebkitOverflowScrolling: "touch" }}>
                {(["yesterday", "today", "month"] as const).map((key) => {
                  const label = key === "yesterday" ? "Yesterday" : key === "today" ? "Today" : "This Month";
                  const isActive = key === subDateFilter;
                  return (
                    <div key={key} onClick={() => { setSubDateFilter(key); }} className={`scroll-snap-item shrink-0 px-[14px] py-[7px] rounded-full text-[13px] cursor-pointer transition-all whitespace-nowrap ${isActive ? "text-white" : "text-white/50"}`} style={{ background: isActive ? "rgb(177, 44, 73)" : "rgba(255,255,255,0.08)" }}>
                      {label}
                    </div>
                  );
                })}
                  ...
                  <div className="van-picker">
                    ...
                    <div className="van-picker__columns">
                      <div className="van-picker-column">
                        <div className="van-picker-column__wrapper" style={{ transform: `translateY(${-((yr - 2020) * 44) + 110}px)` }} onWheel={e => { e.preventDefault(); const el = e.currentTarget; const IH = 44; const ITEMS = Array.from({ length: MAX_YR - 2020 + 1 }, (_, i) => 2020 + i); const MIN = -(ITEMS.length - 1) * IH; if (el.dataset.sr) { cancelAnimationFrame(parseInt(el.dataset.sr)); el.dataset.sr = ''; } let off = el.dataset.so ? parseFloat(el.dataset.so) : -((yr - 2020) * IH); off -= e.deltaY; off = Math.max(MIN, Math.min(0, off)); el.style.transform = `translateY(${off + 110}px)`; el.dataset.so = String(off); el.dataset.sv = String(-e.deltaY * 0.25); const anim = () => { let v = parseFloat(el.dataset.sv || '0'); v *= 0.92; let o = parseFloat(el.dataset.so || '0') + v; o = Math.max(MIN, Math.min(0, o)); if (Math.abs(v) < 0.5) { const i = Math.max(0, Math.min(ITEMS.length - 1, Math.round(-o / IH))); const s = -(i * IH); el.style.transform = `translateY(${s + 110}px)`; el.dataset.so = String(s); setYr(ITEMS[i]); el.dataset.sr = ''; return; } el.style.transform = `translateY(${o + 110}px)`; el.dataset.so = String(o); el.dataset.sv = String(v); el.dataset.sr = String(requestAnimationFrame(anim)); }; el.dataset.sr = String(requestAnimationFrame(anim)); }} onTouchStart={e => { const el = e.currentTarget; if (el.dataset.sr) { cancelAnimationFrame(parseInt(el.dataset.sr)); el.dataset.sr = ''; } if (!el.dataset.so) el.dataset.so = String(-((yr - 2020) * 44)); el.dataset.st = String(e.touches[0].clientY); el.dataset.sl = String(e.touches[0].clientY); el.dataset.sn = String(Date.now()); el.dataset.sv = '0'; }} onTouchMove={e => { const el = e.currentTarget; const IH = 44; const ITEMS = Array.from({ length: MAX_YR - 2020 + 1 }, (_, i) => 2020 + i); const MIN = -(ITEMS.length - 1) * IH; const y = e.touches[0].clientY; const ly = parseFloat(el.dataset.sl || String(y)); const d = y - ly; const n = Date.now(); const dt = n - parseFloat(el.dataset.sn || String(n)); if (dt > 0) el.dataset.sv = String((d / dt) * 16); let o = parseFloat(el.dataset.so || '0') + d; o = Math.max(MIN, Math.min(0, o)); el.style.transform = `translateY(${o + 110}px)`; el.dataset.so = String(o); el.dataset.sl = String(y); el.dataset.sn = String(n); }} onTouchEnd={e => { const el = e.currentTarget; const IH = 44; const ITEMS = Array.from({ length: MAX_YR - 2020 + 1 }, (_, i) => 2020 + i); const MIN = -(ITEMS.length - 1) * IH; const vel = parseFloat(el.dataset.sv || '0'); if (Math.abs(vel) > 0.5) { const anim = () => { let v = parseFloat(el.dataset.sv || '0'); v *= 0.92; let o = parseFloat(el.dataset.so || '0') + v; o = Math.max(MIN, Math.min(0, o)); if (Math.abs(v) < 0.5) { const i = Math.max(0, Math.min(ITEMS.length - 1, Math.round(-o / IH))); const s = -(i * IH); el.style.transform = `translateY(${s + 110}px)`; el.dataset.so = String(s); setYr(ITEMS[i]); el.dataset.sr = ''; return; } el.style.transform = `translateY(${o + 110}px)`; el.dataset.so = String(o); el.dataset.sv = String(v); el.dataset.sr = String(requestAnimationFrame(anim)); }; el.dataset.sr = String(requestAnimationFrame(anim)); } else { const o = parseFloat(el.dataset.so || '0'); const i = Math.max(0, Math.min(ITEMS.length - 1, Math.round(-o / IH))); const s = -(i * IH); el.style.transform = `translateY(${s + 110}px)`; el.dataset.so = String(s); setYr(ITEMS[i]); } }}>
                      ...
                      <div className="van-picker-column">
                        <div className="van-picker-column__wrapper" style={{ transform: `translateY(${-((mo - 1) * 44) + 110}px)` }} onWheel={e => { e.preventDefault(); const el = e.currentTarget; const IH = 44; const ITEMS = Array.from({ length: yr >= MAX_YR ? MAX_MO : 12 }, (_, i) => i + 1); const MIN = -(ITEMS.length - 1) * IH; if (el.dataset.sr) { cancelAnimationFrame(parseInt(el.dataset.sr)); el.dataset.sr = ''; } let off = el.dataset.so ? parseFloat(el.dataset.so) : -((mo - 1) * IH); off -= e.deltaY; off = Math.max(MIN, Math.min(0, off)); el.style.transform = `translateY(${off + 110}px)`; el.dataset.so = String(off); el.dataset.sv = String(-e.deltaY * 0.25); const anim = () => { let v = parseFloat(el.dataset.sv || '0'); v *= 0.92; let o = parseFloat(el.dataset.so || '0') + v; o = Math.max(MIN, Math.min(0, o)); if (Math.abs(v) < 0.5) { const i = Math.max(0, Math.min(ITEMS.length - 1, Math.round(-o / IH))); const s = -(i * IH); el.style.transform = `translateY(${s + 110}px)`; el.dataset.so = String(s); setMo(ITEMS[i]); el.dataset.sr = ''; return; } el.style.transform = `translateY(${o + 110}px)`; el.dataset.so = String(o); el.dataset.sv = String(v); el.dataset.sr = String(requestAnimationFrame(anim)); }; el.dataset.sr = String(requestAnimationFrame(anim)); }} onTouchStart={e => { const el = e.currentTarget; if (el.dataset.sr) { cancelAnimationFrame(parseInt(el.dataset.sr)); el.dataset.sr = ''; } if (!el.dataset.so) el.dataset.so = String(-((mo - 1) * 44)); el.dataset.st = String(e.touches[0].clientY); el.dataset.sl = String(e.touches[0].clientY); el.dataset.sn = String(Date.now()); el.dataset.sv = '0'; }} onTouchMove={e => { const el = e.currentTarget; const IH = 44; const ITEMS = Array.from({ length: yr >= MAX_YR ? MAX_MO : 12 }, (_, i) => i + 1); const MIN = -(ITEMS.length - 1) * IH; const y = e.touches[0].clientY; const ly = parseFloat(el.dataset.sl || String(y)); const d = y - ly; const n = Date.now(); const dt = n - parseFloat(el.dataset.sn || String(n)); if (dt > 0) el.dataset.sv = String((d / dt) * 16); let o = parseFloat(el.dataset.so || '0') + d; o = Math.max(MIN, Math.min(0, o)); el.style.transform = `translateY(${o + 110}px)`; el.dataset.so = String(o); el.dataset.sl = String(y); el.dataset.sn = String(n); }} onTouchEnd={e => { const el = e.currentTarget; const IH = 44; const ITEMS = Array.from({ length: yr >= MAX_YR ? MAX_MO : 12 }, (_, i) => i + 1); const MIN = -(ITEMS.length - 1) * IH; const vel = parseFloat(el.dataset.sv || '0'); if (Math.abs(vel) > 0.5) { const anim = () => { let v = parseFloat(el.dataset.sv || '0'); v *= 0.92; let o = parseFloat(el.dataset.so || '0') + v; o = Math.max(MIN, Math.min(0, o)); if (Math.abs(v) < 0.5) { const i = Math.max(0, Math.min(ITEMS.length - 1, Math.round(-o / IH))); const s = -(i * IH); el.style.transform = `translateY(${s + 110}px)`; el.dataset.so = String(s); setMo(ITEMS[i]); el.dataset.sr = ''; return; } el.style.transform = `translateY(${o + 110}px)`; el.dataset.so = String(o); el.dataset.sv = String(v); el.dataset.sr = String(requestAnimationFrame(anim)); }; el.dataset.sr = String(requestAnimationFrame(anim)); } else { const o = parseFloat(el.dataset.so || '0'); const i = Math.max(0, Math.min(ITEMS.length - 1, Math.round(-o / IH))); const s = -(i * IH); el.style.transform = `translateY(${s + 110}px)`; el.dataset.so = String(s); setMo(ITEMS[i]); } }}>
                      ...
                      <div className="van-picker-column">
                        <div className="van-picker-column__wrapper" style={{ transform: `translateY(${-((dd - 1) * 44) + 110}px)` }} onWheel={e => { e.preventDefault(); const el = e.currentTarget; const IH = 44; const ITEMS = Array.from({ length: (yr === MAX_YR && mo === MAX_MO) ? MAX_DD : [4,6,9,11].includes(mo) ? 30 : mo === 2 ? 28 : 31 }, (_, i) => i + 1); const MIN = -(ITEMS.length - 1) * IH; if (el.dataset.sr) { cancelAnimationFrame(parseInt(el.dataset.sr)); el.dataset.sr = ''; } let off = el.dataset.so ? parseFloat(el.dataset.so) : -((dd - 1) * IH); off -= e.deltaY; off = Math.max(MIN, Math.min(0, off)); el.style.transform = `translateY(${off + 110}px)`; el.dataset.so = String(off); el.dataset.sv = String(-e.deltaY * 0.25); const anim = () => { let v = parseFloat(el.dataset.sv || '0'); v *= 0.92; let o = parseFloat(el.dataset.so || '0') + v; o = Math.max(MIN, Math.min(0, o)); if (Math.abs(v) < 0.5) { const i = Math.max(0, Math.min(ITEMS.length - 1, Math.round(-o / IH))); const s = -(i * IH); el.style.transform = `translateY(${s + 110}px)`; el.dataset.so = String(s); setDd(ITEMS[i]); el.dataset.sr = ''; return; } el.style.transform = `translateY(${o + 110}px)`; el.dataset.so = String(o); el.dataset.sv = String(v); el.dataset.sr = String(requestAnimationFrame(anim)); }; el.dataset.sr = String(requestAnimationFrame(anim)); }} onTouchStart={e => { const el = e.currentTarget; if (el.dataset.sr) { cancelAnimationFrame(parseInt(el.dataset.sr)); el.dataset.sr = ''; } if (!el.dataset.so) el.dataset.so = String(-((dd - 1) * 44)); el.dataset.st = String(e.touches[0].clientY); el.dataset.sl = String(e.touches[0].clientY); el.dataset.sn = String(Date.now()); el.dataset.sv = '0'; }} onTouchMove={e => { const el = e.currentTarget; const IH = 44; const ITEMS = Array.from({ length: (yr === MAX_YR && mo === MAX_MO) ? MAX_DD : [4,6,9,11].includes(mo) ? 30 : mo === 2 ? 28 : 31 }, (_, i) => i + 1); const MIN = -(ITEMS.length - 1) * IH; const y = e.touches[0].clientY; const ly = parseFloat(el.dataset.sl || String(y)); const d = y - ly; const n = Date.now(); const dt = n - parseFloat(el.dataset.sn || String(n)); if (dt > 0) el.dataset.sv = String((d / dt) * 16); let o = parseFloat(el.dataset.so || '0') + d; o = Math.max(MIN, Math.min(0, o)); el.style.transform = `translateY(${o + 110}px)`; el.dataset.so = String(o); el.dataset.sl = String(y); el.dataset.sn = String(n); }} onTouchEnd={e => { const el = e.currentTarget; const IH = 44; const ITEMS = Array.from({ length: (yr === MAX_YR && mo === MAX_MO) ? MAX_DD : [4,6,9,11].includes(mo) ? 30 : mo === 2 ? 28 : 31 }, (_, i) => i + 1); const MIN = -(ITEMS.length - 1) * IH; const vel = parseFloat(el.dataset.sv || '0'); if (Math.abs(vel) > 0.5) { const anim = () => { let v = parseFloat(el.dataset.sv || '0'); v *= 0.92; let o = parseFloat(el.dataset.so || '0') + v; o = Math.max(MIN, Math.min(0, o)); if (Math.abs(v) < 0.5) { const i = Math.max(0, Math.min(ITEMS.length - 1, Math.round(-o / IH))); const s = -(i * IH); el.style.transform = `translateY(${s + 110}px)`; el.dataset.so = String(s); setDd(ITEMS[i]); el.dataset.sr = ''; return; } el.style.transform = `translateY(${o + 110}px)`; el.dataset.so = String(o); el.dataset.sv = String(v); el.dataset.sr = String(requestAnimationFrame(anim)); }; el.dataset.sr = String(requestAnimationFrame(anim)); } else { const o = parseFloat(el.dataset.so || '0'); const i = Math.max(0, Math.min(ITEMS.length - 1, Math.round(-o / IH))); const s = -(i * IH); el.style.transform = `translateY(${s + 110}px)`; el.dataset.so = String(s); setDd(ITEMS[i]); } }}>
              ...
              <>
                ...
                <div className="van-popup van-popup--round van-popup--bottom">
                  <div className="van-picker">
                    ...
                    <div className="van-picker__columns">
                      <div className="van-picker-column">
                        <div className="van-picker-column__wrapper" style={{ transform: `translateY(${-((levelVal === "All" ? 0 : levelVal === "Level 1" ? 1 : levelVal === "Level 2" ? 2 : 3) * 44) + 110}px)` }} onWheel={e => { e.preventDefault(); const el = e.currentTarget; const IH = 44; const ITEMS = ["All","Level 1","Level 2","Level 3"]; const MIN = -(ITEMS.length - 1) * IH; if (el.dataset.sr) { cancelAnimationFrame(parseInt(el.dataset.sr)); el.dataset.sr = ''; } const curIdx = ITEMS.indexOf(levelVal); let off = el.dataset.so ? parseFloat(el.dataset.so) : -(curIdx >= 0 ? curIdx : 0) * IH; off -= e.deltaY; off = Math.max(MIN, Math.min(0, off)); el.style.transform = `translateY(${off + 110}px)`; el.dataset.so = String(off); el.dataset.sv = String(-e.deltaY * 0.25); const anim = () => { let v = parseFloat(el.dataset.sv || '0'); v *= 0.92; let o = parseFloat(el.dataset.so || '0') + v; o = Math.max(MIN, Math.min(0, o)); if (Math.abs(v) < 0.5) { const i = Math.max(0, Math.min(ITEMS.length - 1, Math.round(-o / IH))); const s = -(i * IH); el.style.transform = `translateY(${s + 110}px)`; el.dataset.so = String(s); setLevelVal(ITEMS[i]); el.dataset.sr = ''; return; } el.style.transform = `translateY(${o + 110}px)`; el.dataset.so = String(o); el.dataset.sv = String(v); el.dataset.sr = String(requestAnimationFrame(anim)); }; el.dataset.sr = String(requestAnimationFrame(anim)); }} onTouchStart={e => { const el = e.currentTarget; if (el.dataset.sr) { cancelAnimationFrame(parseInt(el.dataset.sr)); el.dataset.sr = ''; } const curIdx = ["All","Level 1","Level 2","Level 3"].indexOf(levelVal); if (!el.dataset.so) el.dataset.so = String(-(curIdx >= 0 ? curIdx : 0) * 44); el.dataset.st = String(e.touches[0].clientY); el.dataset.sl = String(e.touches[0].clientY); el.dataset.sn = String(Date.now()); el.dataset.sv = '0'; }} onTouchMove={e => { const el = e.currentTarget; const IH = 44; const ITEMS = ["All","Level 1","Level 2","Level 3"]; const MIN = -(ITEMS.length - 1) * IH; const y = e.touches[0].clientY; const ly = parseFloat(el.dataset.sl || String(y)); const d = y - ly; const n = Date.now(); const dt = n - parseFloat(el.dataset.sn || String(n)); if (dt > 0) el.dataset.sv = String((d / dt) * 16); let o = parseFloat(el.dataset.so || '0') + d; o = Math.max(MIN, Math.min(0, o)); el.style.transform = `translateY(${o + 110}px)`; el.dataset.so = String(o); el.dataset.sl = String(y); el.dataset.sn = String(n); }} onTouchEnd={e => { const el = e.currentTarget; const IH = 44; const ITEMS = ["All","Level 1","Level 2","Level 3"]; const MIN = -(ITEMS.length - 1) * IH; const vel = parseFloat(el.dataset.sv || '0'); if (Math.abs(vel) > 0.5) { const anim = () => { let v = parseFloat(el.dataset.sv || '0'); v *= 0.92; let o = parseFloat(el.dataset.so || '0') + v; o = Math.max(MIN, Math.min(0, o)); if (Math.abs(v) < 0.5) { const i = Math.max(0, Math.min(ITEMS.length - 1, Math.round(-o / IH))); const s = -(i * IH); el.style.transform = `translateY(${s + 110}px)`; el.dataset.so = String(s); setLevelVal(ITEMS[i]); el.dataset.sr = ''; return; } el.style.transform = `translateY(${o + 110}px)`; el.dataset.so = String(o); el.dataset.sv = String(v); el.dataset.sr = String(requestAnimationFrame(anim)); }; el.dataset.sr = String(requestAnimationFrame(anim)); } else { const o = parseFloat(el.dataset.so || '0'); const i = Math.max(0, Math.min(ITEMS.length - 1, Math.round(-o / IH))); const s = -(i * IH); el.style.transform = `translateY(${s + 110}px)`; el.dataset.so = String(s); setLevelVal(ITEMS[i]); } }}>
